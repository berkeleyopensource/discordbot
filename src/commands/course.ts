...
export class CourseCommand extends EECSCommand {
        ...
        return fetch('https://berkeleytime.com/api/catalog/catalog_json/')
            ...
            .then(data => {
                ...
                for (const course of data['courses']) {
                    if (!(course['abbreviation'] in newCatalog)) newCatalog[course['abbreviation']] = {}

                    newCatalog[course['abbreviation']][course['course_number']] = course['id']
                }

                if (!this.catalog) console.log('Course catalog initialized!')
    ...
    async execute(message: CommandoMessage, args: string) {
        ...
        for (const abbrev in this.catalog) {
            if (abbrev.length > longestAbbrev.length && search.startsWith(abbrev)) longestAbbrev = abbrev
        }
        ...
        if (!longestAbbrev) return message.say(`Course \`${args}\` not found`)
        ...
        if (courseNum in this.catalog[longestAbbrev]) {
            const response = await fetch(
                `https://www.berkeleytime.com/api/catalog/catalog_json/course_box/?course_id=${this.catalog[longestAbbrev][courseNum]}`
            )
            ...
            if (data['course']['description']) {
                ...
                if (description.length > 256) description = description.substring(0, 256 - 3) + '...'
            }
                ...
                new MessageEmbed({
                    ...
                    fields: [
                        ...
                        },
                        {
                            name: 'Enrolled',
                            inline: true,
                            value: data['course']['enrolled_percentage']
                                ? `${Math.round(data['course']['enrolled_percentage'] * 100)}%`
                                : 'Unknown',
                        },
                    ],
                })
